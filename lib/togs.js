    ...
    if (['MultiPoint', 'LineString'].includes(geometry.type)) {
        ...
        for (var i = 0; i < feature.geometry.coordinates.length; i++ ) {
            ...
            if (!bbox) {
                ...
                bbox.min[0] = Math.min(bbox.min[0], coors[i][0]);
                bbox.min[1] = Math.min(bbox.min[1], coors[i][1]);
                bbox.max[0] = Math.min(bbox.max[0], coors[i][0]);
                bbox.max[1] = Math.min(bbox.max[1], coors[i][1]); 
            }
        }
    }

    if (['MultiLineString','MultiPolygon'].includes(geometry.type)) {
        for (var i = 0; i < feature.geometry.coordinates.length; i++ )
            for (var j = 0; j < feature.geometry.coordinates[i].length; j++) {

                if (!bbox) {
                    bbox={}; 
                    bbox.min = geometry.coors[i][j].slice();
                    bbox.max = geometry.coors[i][j].slice(); 
                } else {
                    bbox.min[0] = Math.min(bbox.min[0], coors[i][j][0]);
                    bbox.min[1] = Math.min(bbox.min[1], coors[i][j][1]);
                    bbox.max[0] = Math.min(bbox.max[0], coors[i][j][0]);
                    bbox.max[1] = Math.min(bbox.max[1], coors[i][j][1]); 
                }
        }
    }
